...
def gauss_ci_test(suff_stat,
                  x: int,
                  y: int,
                  K: List[int],
                  cut_at: float = 0.9999999):
    """条件独立性检验"""
    C = suff_stat["C"]
    n = suff_stat["n"]
...
def extend_cpdag(G, O):
    ...
    def rule1(g):
        ...
        pairs = [(i, j)
                 ...
                 if g[i][j] == 1 and g[j][i] == 0]  # 所有 i - j
        for i, j in pairs:
            all_k = [
                k for k in range(n_nodes) if (g[j][k] == 1 and g[k][j] == 1) and
                (g[i][k] == 0 and g[k][i] == 0)
            ]
            if len(all_k) > 0:
                for all_k_i in all_k:
                    g[j][all_k_i] = 1
                    g[all_k_i][j] = 0
    ...
    def rule2(g):
        ...
        pairs = [(i, j)
                 for i in range(n_nodes)
                 for j in range(n_nodes)
                 if g[i][j] == 1 and g[j][i] == 1]  # 所有 i - j 点对

        for i, j in pairs:
            all_k = [
                k for k in range(n_nodes) if (g[i][k] == 1 and g[k][i] == 0) and
                (g[k][j] == 1 and g[j][k] == 0)
            ]
    ...
    def rule3(g):
        ...
        for i, j in pairs:
            all_k = [
                k for k in range(n_nodes) if (g[i][k] == 1 and g[k][i] == 1) and
                (g[k][j] == 1 and g[j][k] == 0)
            ]
    ...
    # 相邻点对
    pairs = [
        (i, j) for i in range(n_nodes) for j in range(n_nodes) if G[i][j] == 1
    ]
